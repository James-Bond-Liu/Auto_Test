...
data2 = df.loc[[1, 2]].values   #读取指定的多行，数据会存在嵌套的列表里面

data3 = df.iloc[1, 2]   #读取指定的行列

data4 = df.loc[[1, 2], ['case_id','url','http_method']]  #读取指定的多行多列值：

data5 = df.loc[:, ['case_id','data','title','http_method','expected']].values    #获取所有行的指定列,得到的是嵌套列表

data6 = df.loc[:, ['case_id','data','title','http_method','expected']]    #获取所有行的指定列,得到二维矩阵

data7 = df.head(n=5) #获取前几行数据，默认5行

data8 = df.tail(n=5)    #获取后几行数据，默认5行

print("输出行号列表:", df.index.values)   #获取行号并打印输出,从0开始

print("输出标题:", df.columns.values)   #获取列名并打印输出['case_id','data','title','http_method','expected']
