...
class BasePage(object):
    ...
    def click(self, by, locator, open=False):
        """点击某个元素 显示等待"""
        element = None
        try:
    ...
    def js_script(self, js, param=None):
        ...
        # print('info :the "{}" success'.format(js))
        # except Exception as why:  #     print(why)  #     print('the "{}" failed!'.format(js))  # return
    ...
    def check_page(self, home, btn, check_el, checkdata):
        check = []
        self.page_refresh()
        for i, j, z in zip(btn, check_el, checkdata):
    ...
    def login(self, username=login_data.username, password=login_data.password):
        self.load_url(login_data.login_url)
        self.click(*el_login.f_login_btn)
        self.sleep(2)
        self.send_keys(*el_login.email, username)
        self.send_keys(*el_login.password, password)
    ...
    def login_guangdong(self):
        try:
            self.load_url(login_data.guangdong_tax_bureau)
            self.sleep(1)
            # self.click(*el_guangdong.close_tips_btn)
            
            self.click(*el_guangdong.login_btn)
            self.click(*el_guangdong.close_tips_btn2)
            self.sleep(2)
            self.click(*el_guangdong.login)
            self.sleep(2)
            self.send_keys(*el_guangdong.ipt_shxydmOrsbh, login_data.identity_number_taxpayer)
            self.send_keys(*el_guangdong.ipt_userNameOrSjhm, login_data.sj_username)
            self.send_keys(*el_guangdong.ipt_passWord, login_data.sj_password)
            self.switch_slider(*el_guangdong.slider)
            self.click(*el_guangdong.upLoginButton)
        except Exception as why:
            print(why)
    ...
    def _base(func):
        ...
        def wapper(self, *args, **kwargs):
            case_name, check_info = None, None
            try:
                case_name, check_info = func(self, *args, **kwargs)
                print(case_name, "|执行完毕")
            except Exception as why:
                print('why', why)
