...
class GetRequestData():
    def get_filename(self):  # 获取目录下所有文件的文件名
        try:
            for root, dirs, files in os.walk(Path.excel_data):
                logger.info("路径{}下的所有文件为{}".format(Path.excel_data, files))
                return files
        except Exception as e:
            logger.error("获取路径{}下的所有文件失败，错误为{}".format(Path.excel_data, e))
            raise e
    ...
    def get_request_data(self):
        ...
        logger.info(f'目录下{Path.excel_data}的所有测试数据已经全部获取完成')
        return test_data
