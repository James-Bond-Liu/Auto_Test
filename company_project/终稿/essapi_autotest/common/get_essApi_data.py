...
class GetRequestData():
    def get_filename(self):  # 获取目录下所有文件的文件名
        try:
            for root, dirs, files in os.walk(Path.data_essApi):
                logger.info("路径{}下的所有文件为{}".format(Path.data_essApi, files))
                return files
        except Exception as e:
            logger.error("获取路径{}下的所有文件失败，错误为{}".format(Path.data_essApi, e))
            raise e
    ...
    def get_request_data(self):
        ...
        for file in filenames:
            ...
            path2 = os.path.join(path1, 'data_essApi', file)
        ...
        logger.info(f'目录下{Path.data_essApi}的所有测试数据已经全部获取完成')
        return test_data
