...
class Run():
    ...
    def clean_up(self):
        ...
        if os.path.exists(Path.log_path):
            with open(Path.log_path, 'w') as file1:
                file1.truncate(0)
        if os.path.exists(Path.report_thirdApi_html):
            with open(Path.report_thirdApi_html, 'wb') as file2:
                file2.truncate(0)
        if os.path.exists(Path.report_thirdApi_allure):
            shutil.rmtree(Path.report_thirdApi_allure)
        time.sleep(3)
    ...
    def run(self):
        ...
        pytest.main(['--clean-alluredir', f'--html={Path.report_thirdApi_html}', f'--alluredir={Path.report_thirdApi_allure}'])
        ...
        zip_path = os.path.split(Path().report_thirdApi_html)[0]
