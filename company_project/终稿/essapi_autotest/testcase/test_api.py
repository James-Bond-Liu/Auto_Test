...
class TestApi():
    ...
    def test_api(self, item):
        ...
        if item['sql_before'] != 'None':
            ...
            logger.debug(f"{item['case_name']}：{item['sql_before'][2]}")
            result1 = \
        ...
        if key and type(key) != int:
            ...
            # if res.json().get('data').get('access_token'):
        ...
        try:
            ...
            if item['sql_after'] != 'None':
                ...
                # logger.debug(f"{item['sql_after'][1]}")
            ...
            try:
                ...
                # UpdateData().update(file_name, item['sheet_name'], item['case_id'] - 1, 'request_actual_result', str(res.json()))
                UpdateData().update(file_name, item['sheet_name'], item['case_id'] + 1, 8, str(res.json()))
                ...
                UpdateData().update(file_name, item['sheet_name'], item['case_id'] + 1, 13, compare_result)
