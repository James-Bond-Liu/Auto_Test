...
class http_request_login(unittest.TestCase):
    ...
    def setUp(self):
        ...
        '''
        self.login_url = 'http://ip:8080/futureloan/mvc/api/member/login' #只有加上self，在类后面的方法函数中才可以调用这个属性
        self.recharge_url='http://ip:8080/futureloan/mvc/api/member/recharge'

    def test_login_normal(self):
        global COOKIE  #声明此方法中的COOKIE是全局变量，可以修改上面的变量
        ...
        if res.cookies:#如果cookie有的话就更新cookie
            COOKIE=res.cookies#利用全局变量的方法更新cookie
        try:
    ...
    #充值请求的cookie来自于全局变量COOKIE
    def test_recharge_normal(self):
        recharge_data = {'name':'15631128476','amount': '1000'}
        recharge_res=HttpRequest().http_request(self.recharge_url,'post',recharge_data,COOKIE)  #全局变量方法
        try:
